...
namespace {
PythonAdminBridge* g_pythonAdminBridge = nullptr;
MV3DEditorBridge* g_mv3dEditorBridge = nullptr;
}

MV3DEditorBridge* CanaryServer::getMV3DBridge() {
	return g_mv3dEditorBridge;
}
...
CanaryServer::CanaryServer(
	...
	pythonAdminBridge = std::make_unique<PythonAdminBridge>(logger);
	g_pythonAdminBridge = pythonAdminBridge.get();
	mv3dEditorBridge = std::make_unique<MV3DEditorBridge>(logger);
	g_mv3dEditorBridge = mv3dEditorBridge.get();
	...
	g_dispatcher().addEvent(
		[this] {
			try {
				...
				if (pythonAdminBridge) {
					pythonAdminBridge->start();
				}
				if (mv3dEditorBridge) {
					mv3dEditorBridge->start();
				}
...
void CanaryServer::shutdown() {
	if (g_mv3dEditorBridge) {
		g_mv3dEditorBridge->stop();
		g_mv3dEditorBridge = nullptr;
	}

	if (g_pythonAdminBridge) {
		g_pythonAdminBridge->stop();
		g_pythonAdminBridge = nullptr;
	}
