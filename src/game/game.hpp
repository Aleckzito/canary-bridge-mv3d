...
public:
	...
	void broadcastBossPhase(uint32_t bossId, uint8_t phase);
	void setWeather(uint8_t type, uint16_t intensity);
	void setLightOverride(uint8_t level, uint8_t color);
	void sendSoundEvent(uint16_t soundId, const Position &pos, uint8_t volume);
...
private:
	uint8_t lightOverrideLevel = 0;
	uint8_t lightOverrideColor = 0;
};
