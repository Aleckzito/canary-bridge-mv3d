...
void Game::checkLight() {
	...
	if (lightChange) {
		if (auto* bridge = CanaryServer::getMV3DBridge()) {
			bridge->broadcastWorldLight(lightInfo.level, lightInfo.color);
		}
...
LightInfo Game::getWorldLightInfo() const {
	if (lightOverrideLevel > 0) {
		return { lightOverrideLevel, lightOverrideColor };
	}
...
void Game::broadcastBossPhase(uint32_t bossId, uint8_t phase) {
	if (auto* bridge = CanaryServer::getMV3DBridge()) {
		bridge->broadcastBossPhaseSync(bossId, phase, 0); // HP logic can be added later
	}
}

void Game::setWeather(uint8_t type, uint16_t intensity) {
	if (auto* bridge = CanaryServer::getMV3DBridge()) {
		bridge->broadcastWeatherSync(type, intensity);
	}
}

void Game::setLightOverride(uint8_t level, uint8_t color) {
	lightOverrideLevel = level;
	lightOverrideColor = color;

	if (auto* bridge = CanaryServer::getMV3DBridge()) {
		LightInfo info = getWorldLightInfo();
		bridge->broadcastWorldLight(info.level, info.color);
	}
}

void Game::sendSoundEvent(uint16_t soundId, const Position &pos, uint8_t volume) {
	if (auto* bridge = CanaryServer::getMV3DBridge()) {
		bridge->broadcastSoundEvent(soundId, pos.x, pos.y, pos.z, volume);
	}
}

void Game::setWorldFlag(const std::string &key, bool value) {
	if (auto* bridge = CanaryServer::getMV3DBridge()) {
		bridge->broadcastWorldFlagDelta(key, value);
	}
}
