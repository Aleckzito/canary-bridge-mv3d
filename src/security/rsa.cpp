...
void RSAManager::encryptPublic(char* msg) const {
	// Operación de clave pública: c = m^e mod n  (e = 65537)
	// Usada por clientes para cifrar nonces y verificar firmas del server.
	static thread_local OpenSSLPtr<BN_CTX> ctx(BN_CTX_new(), BN_CTX_free);
	if (!ctx) {
		ctx.reset(BN_CTX_new());
		if (!ctx) {
			logger.error("RSAManager::encryptPublic: failed to allocate BN_CTX");
			return;
		}
	}
	...
	struct BNCTXFrame {
		BN_CTX* c;
		explicit BNCTXFrame(BN_CTX* c) : c(c) {}
		~BNCTXFrame() { BN_CTX_end(c); }
	} frame(ctx.get());
	...
	if (!m || !c || !e) {
		logger.error("RSAManager::encryptPublic: BN_CTX_get failed");
		return;
	}

	if (!BN_bin2bn(reinterpret_cast<const unsigned char*>(msg), 128, m)) {
		logger.error("RSAManager::encryptPublic: BN_bin2bn failed");
		return;
	}

	if (!BN_set_word(e, 65537)) {
		logger.error("RSAManager::encryptPublic: BN_set_word(e=65537) failed");
		return;
	}

	if (!BN_mod_exp_mont(c, m, e, n.get(), ctx.get(), mont_ctx.get())) {
		logger.error("RSAManager::encryptPublic: BN_mod_exp_mont failed");
		return;
	}

	const int num_bytes = BN_num_bytes(c);
	const int padding   = 128 - num_bytes;
	if (padding < 0) {
		logger.error("RSAManager::encryptPublic: result exceeds 128 bytes");
		return;
	}
	if (padding > 0) {
		std::memset(msg, 0, static_cast<size_t>(padding));
	}
	BN_bn2bin(c, reinterpret_cast<unsigned char*>(msg) + padding);
}
