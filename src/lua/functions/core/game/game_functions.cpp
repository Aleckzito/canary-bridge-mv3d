...
void GameFunctions::init(lua_State* L) {
	...
	Lua::registerMethod(L, "Game", "setWeather", GameFunctions::luaGameSetWeather);
	Lua::registerMethod(L, "Game", "setLightOverride", GameFunctions::luaGameSetLightOverride);
	Lua::registerMethod(L, "Game", "broadcastBossPhase", GameFunctions::luaGameBroadcastBossPhase);
	Lua::registerMethod(L, "Game", "broadcastChatMessage", GameFunctions::luaGameBroadcastChatMessage);
	Lua::registerMethod(L, "Game", "playSound", GameFunctions::luaGamePlaySound);
	Lua::registerMethod(L, "Game", "setWorldFlag", GameFunctions::luaGameSetWorldFlag);
}
...
int GameFunctions::luaGameSetWeather(lua_State* L) {
	// Game.setWeather(type, intensity)
	const uint8_t type = Lua::getNumber<uint8_t>(L, 1);
	const uint16_t intensity = Lua::getNumber<uint16_t>(L, 2);
	g_game().setWeather(type, intensity);
	Lua::pushBoolean(L, true);
	return 1;
}

int GameFunctions::luaGameSetLightOverride(lua_State* L) {
	// Game.setLightOverride(level, color)
	const uint8_t level = Lua::getNumber<uint8_t>(L, 1);
	const uint8_t color = Lua::getNumber<uint8_t>(L, 2);
	g_game().setLightOverride(level, color);
	Lua::pushBoolean(L, true);
	return 1;
}

int GameFunctions::luaGameBroadcastBossPhase(lua_State* L) {
	// Game.broadcastBossPhase(bossId, phase)
	const uint32_t bossId = Lua::getNumber<uint32_t>(L, 1);
	const uint8_t phase = Lua::getNumber<uint8_t>(L, 2);
	g_game().broadcastBossPhase(bossId, phase);
	Lua::pushBoolean(L, true);
	return 1;
}

int GameFunctions::luaGameBroadcastChatMessage(lua_State* L) {
	// Game.broadcastChatMessage(senderId, message)
	const uint16_t senderId = Lua::getNumber<uint16_t>(L, 1);
...
int GameFunctions::luaGamePlaySound(lua_State* L) {
	// Game.playSound(soundId, position, volume)
	const uint16_t soundId = Lua::getNumber<uint16_t>(L, 1);
	const Position &pos = Lua::getPosition(L, 2);
	const uint8_t volume = Lua::getNumber<uint8_t>(L, 3, 255);
	g_game().sendSoundEvent(soundId, pos, volume);
	Lua::pushBoolean(L, true);
	return 1;
}

int GameFunctions::luaGameSetWorldFlag(lua_State* L) {
	// Game.setWorldFlag(key, value)
	const std::string &key = Lua::getString(L, 1);
	const bool value = Lua::getBoolean(L, 2);
	g_game().setWorldFlag(key, value);
	Lua::pushBoolean(L, true);
	return 1;
}
