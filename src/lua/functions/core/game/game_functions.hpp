...
private:
	...
	static int luaGameSetWeather(lua_State* L);
	static int luaGameSetLightOverride(lua_State* L);
	static int luaGameBroadcastBossPhase(lua_State* L);
	static int luaGameBroadcastChatMessage(lua_State* L);
	static int luaGamePlaySound(lua_State* L);
	static int luaGameSetWorldFlag(lua_State* L);
};
