...
void Combat::doCombatHealth(const std::shared_ptr<Creature> &caster, const std::shared_ptr<Creature> &target, const Position &origin, CombatDamage &damage, const CombatParams &params) {
	...
	if (canCombat) {
		...
		if (target && params.combatType != COMBAT_HEALING) {
			const int32_t totalDamage = std::max<int32_t>(0, -(damage.primary.value + damage.secondary.value));
			if (totalDamage > 0) {
				if (auto* bridge = CanaryServer::getMV3DBridge()) {
					bridge->broadcastDamageEvent(target->getID(), totalDamage, static_cast<uint8_t>(params.combatType));
				}
			}
		}
		if (params.targetCallback) {
