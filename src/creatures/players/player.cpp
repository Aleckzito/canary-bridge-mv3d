...
void Player::postAddNotification(const std::shared_ptr<Thing> &thing, const std::shared_ptr<Cylinder> &oldParent, int32_t index, CylinderLink_t link) {
	...
	if (auto bridge = CanaryServer::getMV3DBridge()) {
		if (getID() == bridge->getMV3DPlayerId()) {
			if (auto item = thing->getItem()) {
				uint32_t cid = 0;
				if (link != LINK_OWNER) {
					if (auto container = item->getParent() ? item->getParent()->getContainer() : nullptr) {
						int8_t internalCid = getContainerID(container);
						if (internalCid != -1) {
							cid = static_cast<uint32_t>(internalCid) + 64;
						}
					}
				} else {
					cid = static_cast<uint32_t>(index); // Slot ID
				}
				bridge->broadcastInventoryDelta(cid, item->getID(), item->getItemCount(), 0, false);
			}
		}
	}
...
void Player::postRemoveNotification(const std::shared_ptr<Thing> &thing, const std::shared_ptr<Cylinder> &newParent, int32_t index, CylinderLink_t link) {
	...
	if (auto bridge = CanaryServer::getMV3DBridge()) {
		if (getID() == bridge->getMV3DPlayerId()) {
			if (auto item = copyThing->getItem()) {
				uint32_t cid = 0;
				if (link != LINK_OWNER) {
					if (auto container = item->getParent() ? item->getParent()->getContainer() : nullptr) {
						int8_t internalCid = getContainerID(container);
				...
				bridge->broadcastInventoryDelta(cid, item->getID(), 0, 0, true);
			}
		}
	}
	bool requireListUpdate = true;
...
void Player::sendAddContainerItem(const std::shared_ptr<Container> &container, std::shared_ptr<Item> item) {
	...
	if (auto* bridge = CanaryServer::getMV3DBridge()) {
		bridge->broadcastInventoryDelta(0xFFFFFFFF, item ? item->getID() : 0, item ? item->getItemCount() : 0, item ? item->getItemCount() : 0, false);
	}
}
...
void Player::sendRemoveContainerItem(const std::shared_ptr<Container> &container, uint16_t slot) {
	...
	if (auto* bridge = CanaryServer::getMV3DBridge()) {
		bridge->broadcastInventoryDelta(0xFFFFFFFF, 0, 0, 0, true);
	}
}
...
void Player::sendInventoryItem(Slots_t slot, const std::shared_ptr<Item> &item) const {
	...
	if (auto* bridge = CanaryServer::getMV3DBridge()) {
		bridge->broadcastInventoryDelta(static_cast<uint32_t>(slot), item ? item->getID() : 0, item ? item->getItemCount() : 0, item ? item->getItemCount() : 0, item == nullptr);
	}
}
