...
public:
	...
	uint32_t getFirstInteractingPlayerId() const {
		return playerInteractionsOrder.empty() ? 0 : playerInteractionsOrder.back();
	}
	...
	void onDialogNode(uint32_t nodeId, bool closeDialog);
	void onDialogChoice(const std::shared_ptr<Player> &player, uint32_t nodeId, uint8_t optionIndex);
