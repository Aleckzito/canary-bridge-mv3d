...
void Npc::setPlayerInteraction(uint32_t playerId, uint16_t topicId /*= 0*/) {
	...
	onDialogNode(topicId, false);
}

void Npc::removePlayerInteraction(const std::shared_ptr<Player> &player) {
	const bool wasInteracting = playerInteractions.count(player->getID()) > 0;
...
void Npc::onDialogNode(uint32_t nodeId, bool closeDialog) {
	if (auto* bridge = CanaryServer::getMV3DBridge()) {
		bridge->broadcastDialogState(nodeId, closeDialog);
	}
}

void Npc::onDialogChoice(const std::shared_ptr<Player> &player, uint32_t nodeId, uint8_t optionIndex) {
	if (!player) {
		return;
	}
